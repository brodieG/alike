...
```
`alike` treats integer-like numerics as integers.  Additionally, integers can match numeric templates.  If this is not the desired behavior you can use the `int.mode` parameter to engage a stricter comparison mode.  While this feature may seem superfluous it avoids convolutions such as:
```{r, eval=FALSE}
